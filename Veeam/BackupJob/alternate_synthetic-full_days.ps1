...
foreach ($job in $jobs) {
  write-host $job.Name $days[$d]
   Set-VBRJobAdvancedBackupOptions -Job $job -Algorithm Incremental -TransformIncrementsToSyntethic $true -TransformIncrementsToSyntethic $false -TransformToSyntethicDays $days[$d] -EnableFullBackup $false  $d++
  if ($d -ge $days.count) { $d = 0 }
}
